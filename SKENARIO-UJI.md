...
| U-03 | Santri **belum ditempatkan** di kelas | `mudir@ppiq368.sch.id` | Login → **Data Akademik → Kelas** | Bilal Arrahman Saputra (20262005) tampil pada daftar "Santri belum ditempatkan" dan dapat ditempatkan ke kelas mana pun |
| U-04 | Mapel **tanpa ustadz pengampu** | `mudir@ppiq368.sch.id` | Login → **Data Akademik → Mapel & Pengampu** → cari "Komputer" kelas **SD-A** | Kolom pengampu kosong / "Belum ada pengampu"; mapel tetap dapat disunting untuk menetapkan pengampu |
| U-05 | Mapel **tanpa peserta** | `mudir@ppiq368.sch.id` | Login → **Data Akademik → Peserta Mapel** → kelas **SMA-A** → mapel **Berkuda** | Jumlah peserta aktif 0; tombol pendaftaran massal tersedia. Mapel ini juga tidak muncul sebagai baris pada rapor santri SMA-A |
...
| U-15 | Mapel dengan **total bobot komponen ≠ 100** | `mudir@ppiq368.sch.id` | Login → **Data Akademik → Bobot Komponen Nilai** → cari **Fisika (SMP-A)** | Ditandai bermasalah: UTS 40 + UAS 50 = **90%**. Menyimpan bobot yang tidak berjumlah 100% ditolak dengan pesan "Total bobot komponen … harus 100%"; mudir juga dapat menambah komponen baru langsung dari tab ini |
| U-16 | **Penanda tangan rapor** dapat diubah administrasi | `administrasi@ppiq368.sch.id` | Login → **Data Akademik → Penanda Tangan Rapor** → ubah nama/jabatan → **Simpan Perubahan** | Nilai tersimpan pada tabel `AppSetting`; lembar rapor mana pun (mis. rapor Fatih Ahmad Basri) langsung mencetak nama & jabatan yang baru |
| U-17 | Taksonomi kelompok penilaian sesuai naskah | `mudir@ppiq368.sch.id` | Login → **Data Akademik → Kelompok Penilaian** | Terdapat empat kelompok: Akademik, Akhlak (sikap), Tahfidz, Ekstrakurikuler; setiap kelompok dapat disunting/ditambah tanpa mengubah kode |
